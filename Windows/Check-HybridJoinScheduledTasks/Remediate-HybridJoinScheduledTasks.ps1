...
Log-Action -Message "Starting Remediation..."
...
foreach ($CheckTask in $CheckTasks) {
    ...
    Try {
        ...
        if ($TaskObject.State -eq "Disabled") {
            Try {
                Enable-ScheduledTask -TaskName $CheckTask -TaskPath "Microsoft\Windows\Workplace Join" -ErrorAction Stop | Out-Null
                Log-Action -Message "Scheduled Task has been Enabled ($($CheckTask))."
            } catch {
                Log-Action -Message "Error: Failed to enable Scheduled Task ($($CheckTask) - ($($psItem.ToString())))."
                $AllEnabled = $False
            }
        } else {
            Log-Action -Message "Scheduled Task state was not Disabled ($($CheckTask))."
        }
    } catch {
...
if (($AllChecked -eq $True) -and ($AllEnabled -eq $True)) {
    Log-Action -Message "Remediation Result: Success"
} else {
    Log-Action -Message "Remediation Result: Fail"
}
    Log-Action -Message "Finished Remediation."