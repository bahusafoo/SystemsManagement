...
[string]$OrganizationNamedImage = "2107_a" # Update this with the ImageName you are deploying out for newly imaged machines
...
function Update-ImageKeyInfo {
	...
	# Check for OS Build newer than 1909 (MS changed reg value)
	$CurrentWindowsReleaseID = ""
	Try
	{
		if (!($CurrentWindowsReleaseID = $CurrentWindowsVersionKey.GetValue("DisplayVersion")))
		{
